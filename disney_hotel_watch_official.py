...
# -*- coding: utf-8 -*-
"""
⚠️【このスクリプトは使えません／開発中止】⚠️
============================================================================
公式サイト（Akamai Bot Manager）は「自動操縦されたブラウザ」を正確に検知し、
狙い撃ちでアクセスを拒否します。同じ回線・同じPCでも、手で操作するブラウザは
通り、このスクリプトが操作するブラウザだけが即座にブロックされることを確認済みです。

これを回避するには検知逃れの偽装が必要で、サイトの明確な意思表示に反するため
実装しません。公式サイトの空室確認は、以下のどちらかで行ってください。

  ・check_now.py        … 見たい日程を自分のブラウザで一気に開く（手動確認のおたすけ）
  ・test_rakuten.py     … 楽天トラベル公式APIを使う方式の接続テスト（自動監視向け）

以下のコードは経緯の記録として残していますが、実行しても大半がブロックされます。
============================================================================
